...
def sort_products_by_category():
    ...
    """
    # First, get unique categories and sort them alphabetically
    categories = sorted(set(p['category'] for p in products))
    
    result = {}
    for category in categories:
        # Get all products in this category and sort by price
        category_products = [
            p for p in products if p['category'] == category
        ]
        category_products.sort(key=lambda p: p['price'])
...
def browse_products(
    keyword: str = Query(None, description='Search term for product names (case-insensitive)'),
    ...
    """
    Browse products with optional search, sorting, and pagination.
    Order of operations:
    ...
    reverse = (order == 'desc')
    sorted_products = sorted(filtered_products, key=lambda p: p[sort_by], reverse=reverse)
    
    # Step 3: Paginate results
    total_found = len(sorted_products)
    start = (page - 1) * limit
    end = start + limit
    paged_products = sorted_products[start:end]
    
    # Calculate total pages
    total_pages = (total_found + limit - 1) // limit  # Ceiling division
    
    return {
        "keyword": keyword if keyword else "No keyword filter",
        "sort_by": sort_by,
        "order": order,
        "pagination": {
            "page": page,
...
def search_orders(
    customer_name: str = Query(..., description='Search orders by customer name (case-insensitive, partial match)')
):
    """
    Search for orders by customer name.
    - Case-insensitive search
    - Returns partial matches (if search term is contained in customer name)
    - Returns friendly message if no orders found
    """
    matching_orders = [
...
# ── New: Orders Pagination Endpoint ───────────────────────────────────
@app.get('/orders/page')
def get_orders_paged(
    ...
    Returns a page of orders with pagination metadata.
    """
